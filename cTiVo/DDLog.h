...
/* 
 Changes from stock CocoaLumberjack code:
1) REPLACED:
 ERROR => REPORT
 WARN ==> MAJOR
 INFO ==> DETAIL
 ...
 VERBOSE is reused
 --
2) ADDED: in Registered Dynamic Logging section
 + (void)setAllClassesLogLevelFromUserDefaults: (NSString *)defaultsKey;
 + (void) writeAllClassesLogLevelToUserDefaults;

3) ADDED Definitions of those in DDLog.m
 
4)  Added __DDLOGHERE__ at very end of .h :
 ...
 5) Replaced use of LOG_FLAG_ERROR and LOG_FLAG_WARN with LOG_FLAG_REPORT and LOG_FLAG_MAJOR
 ...
 * 
 * NSLog(@"Fatal error, no dohickey found!"); -> DDLogReport(@"Fatal error, no dohickey found!");
 * 
 ...
 * 
 * For example: DDLogMajor(@"%@: Unable to find thingy", THIS_FILE) -> @"MyViewController: Unable to find thingy"
**/
 ...
 * 
 * For example: DDLogMajor(@"%@ - Requires non-nil strings", THIS_METHOD) -> @"setMake:model: requires non-nil strings"
 * 
